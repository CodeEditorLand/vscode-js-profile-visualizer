...
import {
	IVscodeApi,
	VsCodeApi,
} from "vscode-js-profile-core/out/esm/client/vscodeApi";
...
const makeBaseFlame =
	...
	({ columns, range, unit, filtered, Tooltip, InfoBox }) => {
		...
		const [hovered, setHovered] = useState<
			{ box: IBox; src: HighlightSource } | undefined
		>(undefined);
		...
		useEffect(() => {
			...
			requestAnimationFrame(() => {
				...
				for (const box of rawBoxes.boxById.values()) {
					...
					const xScale =
						canvasSize.width / (bounds.maxX - bounds.minX);
				...
				switch (evt.key) {
					case "ArrowRight": {
						for (
							...
							x < columns.length &&
							columns[x].x1 + epsilon(bounds) < bounds.maxX;
							x++
						) {
							const box = getBoxInRowColumn(
								columns,
								rawBoxes.boxById,
								x,
								focused.row,
					...
					case "ArrowLeft": {
						for (
							let x = focused.column - 1;
							x >= 0 &&
							columns[x].x2 - epsilon(bounds) > bounds.minX;
							x--
						) {
							const box = getBoxInRowColumn(
								columns,
		...
		const getBoxUnderCursor = useCallback(
			(evt: MouseEvent) => {
				...
				const { top, left, width } =
					webCanvas.current.getBoundingClientRect();
				...
				const x =
					(fromLeft / width) * (bounds.maxX - bounds.minX) +
					bounds.minX;
				const col =
					Math.abs(binarySearch(columns, (c) => c.x2 - x)) - 1;
		...
		useEffect(() => {
			...
			const { original, pageXOrigin, xPerPixel, pageYOrigin, lock } =
				drag;
			const onMove = (evt: MouseEvent) => {
				...
				if (lock & LockBound.MinX) {
					...
					const upper =
						lock & LockBound.MaxX
							? bounds.maxX - Constants.MinWindow
							: 1 - range;
					minX = clamp(
						0,
		...
		const onWheel = useCallback(
			(evt: WheelEvent) => {
				...
				if (evt.altKey) {
					setBounds({
						...bounds,
						y: clamp(0, bounds.y + evt.deltaY, clampY),
					});
					return;
				}
