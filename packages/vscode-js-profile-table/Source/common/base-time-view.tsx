...
import {
	...
	removeFromSet,
	toggleInSet,
} from "vscode-js-profile-core/out/esm/array";
...
export const makeBaseTimeView =
	...
	({ data, header, query, sortFn, row: Row }) => {
		...
		const [childLoads, setChildLoads] = useState<
			ReadonlyMap<T, Promise<void>>
		>(new Map());
		...
		const sorted = useMemo(() => {
			const topLevel = sortFn
				? data.loaded.slice().sort(sortFn)
				: data.loaded;
		...
		const rendered = useMemo(() => {
			...
			for (let i = 0; i < output.length; i++) {
				...
				if (expanded.has(node)) {
					const children = providers.current
						.get(node)
						?.getChildren(node);
					if (children) {
						...
						for (const child of children.loaded) {
							if (
								query.all ||
								query.selectedAndParents.has(child) ||
								entireSubtree
							) {
								toAdd.push({
									node: child,
		...
		const onKeyDown = useCallback(
			(evt: KeyboardEvent, node: T) => {
				...
				switch (evt.key) {
					...
					case "End":
						if (listRef.current) {
							listRef.current.base.scrollTop =
								listRef.current.base.scrollHeight;
						}
		...
		useEffect(() => {
			setChildLoads((prev) => {
				...
				for (const node of expanded) {
					...
					if (
						children &&
						!children.didReadUpTo(DEFAULT_CHILDREN_LOAD_LEN)
					) {
						next ??= new Map(prev);
		...
		useEffect(
			() => listRef.current?.base.setAttribute("role", "tree"),
			[listRef.current]
		);
		...
		const onLoadMore = useCallback(
			(nodeUn: unknown, dataProvider: DataProvider<unknown>) => {
